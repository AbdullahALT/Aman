 ...
 */
class getSaltRequest extends ServerRequest {

    @Override
    protected void checkQuery(Map<String, String> query) {
        if (query.size() != 1) {
            throw new InvalidNumberOfParametersException("sendRequest::CreateUserRequest accepts 1 parameter " +
                    " in the order [email]");
    ...
    void initRequest(Map<String, String> query) {
        ...
        call = requests.getSalt(query.get("email"));
    }
}
